...
class MarkMovieAsFavoriteIntegrationTest {

  @Container
  private static final PostgreSQLContainer container =
      new PostgreSQLContainer(POSTGRESQL_DOCKER_IMAGE_NAME);

  @Autowired private MarkMovieAsFavorite markMovieAsFavorite;
  ...
  @Test
  void givenAMovieIdThatDoesNotExist_itShouldThrow() {
    Assertions.assertThatThrownBy(() -> markMovieAsFavorite.process(1000L))
  ...
  void givenAMovieThatExists_whenTheMovieIsNotAlreadyMarked_itShouldMarkTheMovieAsFavorite() {
    markMovieAsFavorite.process(1L);
